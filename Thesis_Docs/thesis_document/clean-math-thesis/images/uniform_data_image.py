...
z = np.cos(theta)
theta_ini = (0, 0.61*np.pi, 0.61*np.pi, 0.61*np.pi)
theta_fin = (0.61*np.pi, 0.61*np.pi, 0.61*np.pi, 0)
phi_ini = (0, 0, 0, np.pi)
phi_fin = (0, 0, np.pi, np.pi) #5*np.pi/16
...
Npoints = 1000
z_points = np.random.uniform(1, np.cos(0.61*np.pi), Npoints)
phi_points = np.random.uniform(0, np.pi, Npoints)
...
ax.scatter(x_points, y_points, z_points, color = "orange", s=5)
