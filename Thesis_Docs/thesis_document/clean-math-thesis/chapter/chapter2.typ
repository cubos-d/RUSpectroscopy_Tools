...
$ W_("ij") = (partial u_i) / (partial r_j). $<eq:strain_components>
...
$ epsilon_("ij") = 1/2 ((partial u_i) / (partial r_j) + (partial u_j) / (partial r_i)) $<eq:strain_tensor>
...
Just like the strain tensor, the stress tensor is also symmetric, as long as no torques are applied to the infinitesimal element of our solid. That is, $sigma_("ij") = sigma_("ji")$. In this way, the stress applied to our solid sample is related to the strain through the generalized Hooke's law, which states the following #cite(<Leisure_1997>):

$ sigma_("ij") = sum_(k=0)^2 sum_(l=0)^2 C_("ijkl") epsilon_("kl"). $ <eq:hookes_law>

Here $C_("ijkl")$ are the elastic constants. As we will see later, the elastic constants correspond to the second derivative of the free energy with respect to strain. Apparently, there are 81 independent elastic constants. However, it is possible to simplify. On one hand, the strain tensor is symmetric: $epsilon_("kl") = epsilon_("lk")$. This means that we can determine the same component $sigma_("ij")$ of the stress tensor as follows:

$ sigma_("ij") = sum_(k=0)^2 sum_(l=0)^2 C_("ijkl") epsilon_("kl") = sum_(k=0)^2 sum_(l=0)^2 C_("ijkl") epsilon_("lk"), $

which implies that:

$ C_("ijkl") = C_("ijlk"). $<eq:first_constant_reduction>

As mentioned earlier, the stress matrix is also symmetric in the absence of torques, $sigma_("ij") = sigma_("ji")$. Applying the generalized Hooke’s law to both sides of the equation, we have: 

$ sum_(k=0)^2 sum_(l=0)^2 C_("ijkl") epsilon_("kl") = sum_(k=0)^2 sum_(l=0)^2 C_("jikl") epsilon_("kl"),  $

which implies that:

$ C_("ijkl") = C_("jikl"). $<eq:second_constant_reduction>
...
$ upsilon = 1/2 sum_(m=0)^5 sum_(n=0)^5 C_("mn")epsilon_(m)epsilon_(n). $<eq:potential_energy_density>

From this, it can be seen that the indices n and m can be interchanged, which implies that $ C_("mn") = C_("nm")$. Returning to the old notation, we have
$ C_("ijkl") = C_("klij"). $<eq:third_contant_reduction> 

Thus the tensor $arrow.l.r(C)$ has 21 independent constants in the most general case. 

TODO: Explain the relations (inequalities) that have to be in the constants for stability conditions in different crystaline structures, found in the Mouhat paper. 
