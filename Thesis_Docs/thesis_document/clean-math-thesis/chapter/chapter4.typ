= Linear regression-based approaches for elastic constant prediction<chap:failure>
...
The results for the predictive performance of the linear regression model on the $K$ target are shown in @table:resultados_failure_1.
...
Before analyzing the results, let us define the evaluation metrics used in the table. RMSE stands for Root Mean Squared Error, and it is defined as:

$ "RMSE" = sqrt(1/N_D sum_(j = 1)^(N_D)(y_j - hat(y)_j)^2). $<eq:RMSE_definition>

Here, $N_D$ is the number of data entries, $y_j$ is the true value of the target, and $hat(y)_j$ is the predicted value. The RMSE is also the objective function minimized when fitting a linear regression model using least squares.

To provide a more interpretable measure of model performance, we also use the Mean Absolute Error (MAE), which represents the average absolute difference between the predicted and actual values. It is defined as:

$ "MAE" = 1/N_D sum_(j = 1)^(N_D) abs(y_j - hat(y)_j). $<eq:MAE_DEFINITIONS>

Another metric used to estimate the expected percentage error is the Mean Absolute Percentage Error (MAPE), defined as:

$ "MAPE" = 1/N_D sum_(j=1)^(N_D) abs((y_j - hat(y)_j)/y_j). $<eq:MAPE_definition>

The final metric shown in @table:resultados_failure_1 is the coefficient of determination, denoted as $R^2$. This metric indicates how well the model explains the variance in the target variable. In other words, it measures the proportion of variability in the target that is captured by the model. For linear regression, a high $R^2$ value suggests that the data exhibits a strong linear relationship with the input features, and that the model is effectively capturing that trend.

In the case of the first linear model, we could expect an error of 0.534 $"Tdyn"/"cm"^2$ or 53.4 $"GPa"$ for using the model. This is a huge error considering that, for example, this is approximately the difference between the constant $C_11$ of silver and the constant $C_11$ of NaCl, as we can see in @apx:cubic_constants. Also, the MAPE indicates that we can expect an average relative error of 40% when predicting the elastic constant, which is not experimentally acceptable. In practice, such a high error would significantly reduce the reliability of the model for materials characterization.

On the other hand, the $R^2$ value suggests that $K$ exhibits an approximately linear relationship with the transformed features, particularly with the logarithms of the resonance frequencies $log(omega_n)$. This is further supported by the statistical summary in @fig:statsmodel_results, which shows the results of hypothesis testing for each feature in the linear regression model.
...
As shown in @fig:statsmodel_results, all the p-values associated with the frequency-related features are below 0.05. This indicates that, statistically, the coefficients corresponding to each frequency are significantly different from zero, and therefore each frequency contributes meaningfully to the model. In other words, all frequency features are relevant and should be retained in the model.

To conclude the analysis of the linear model predicting $K$, @fig:residuals displays the distribution of the model’s residuals. We observe that the residuals follow an approximately normal distribution, which suggests that the model satisfies the assumption of homoscedasticity—that is, the variance of the errors remains consistent across all predictions.
...
An independent linear regression model was also trained to predict $G$ using the combinatorial dataset. The performance metrics for this model are presented in @table:resultados_failure_G.
...
The performance metrics of the linear model predicting $G$ are very similar to those of the model predicting $K$, and the analysis previously presented applies here as well. The errors associated with the $G$ model are slightly lower, which may be expected: shear deformation (associated with $G$) generally requires less energy than volumetric deformation (associated with $K$), and therefore may be more strongly influenced by the lower resonance frequencies.

These results suggest that a more expressive model—one that captures nonlinear relationships and includes interactions among input features such as the sample dimensions—is necessary to accurately predict both $K$ and $G$. To explore this, several polynomial regression models were trained, incorporating squared terms, cross-products, and higher-order interactions among the features. An example of a polynimial a model is shown in @eq:poly_regression_1:
...
Several polynomial models were trained using features of increasing complexity, including polynomial terms up to degree 5 and interaction terms such as $sqrt(rho) log(omega_0)$, $log(omega_1) log(omega_2)$, $log(omega_0)^3 log(omega_5)^2$, etc. However, none of these models showed a significant improvement in performance. For example, one model that included polynomial terms for $L_x$, $L_y$, and $L_z$ up to degree 5 achieved an $R^2$ of only 0.27 on the training set, and just 0.05 on the test set—indicating both a poor fit and clear overfitting, due to the large discrepancy between training and test performance. In fact, some models overfitted the training data without even achieving an $R^2$ greater than 0.3, which further highlights their ineffectiveness.

The results presented in this chapter suggest that a different strategy is required to solve the inverse problem effectively. @chap:transformations introduces a new approach that significantly improves predictive performance—achieving substantially lower MAPE values—and provides a more robust solution to the inverse problem.
/*
