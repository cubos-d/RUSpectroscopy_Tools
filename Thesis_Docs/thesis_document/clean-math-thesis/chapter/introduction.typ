...
On the other hand, the inverse problem consists in getting the elastic constants from the frequencies, the mass and the geometry of the sample, as shown in @fig:diagram_inverse_basic. Solving this inverse problem for cubic solids, using a machine learning model, is the objective of the present work. Traditionally the inverse problem have been solved by iterative methods that solve the forward problem repeatedly until we get the constants that yield the minimum error between the measured frequencies and the frequencies got in the last forward problem performed. One of such methods if the Levenberg-Marquardt algorithm which is non-linear least squares method #cite(<Fukuda_2023>). However, this method has some limitations, one of them is the time that take even actual computers to solve it. The other limitation is that a good initial guess of the constants is needed, which requires approximate values of the constants to be known in advance #cite(<Fukuda_2023>). 
