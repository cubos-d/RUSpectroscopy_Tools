...
If the elastic constants of the material are known in advance, the resonance frequencies can be readily predicted by solving a generalized eigenvalue problem (as presented in @eq:raw_eig_problem), which will be described in detail in @chap:forward. The process of computing the resonance frequencies from the elastic constants, the mass, and the geometry of the sample is known as the forward problem:
...
To the best of our knowledge, the only work that proposed a general model capable of predicting the elastic constants for arbitrary cubic materials was the study by Fukuda et al. #cite(<Fukuda_2023>). This work serves as the primary reference and benchmark for the present study.
...
The present work introduces a more general method that can handle samples with any aspect ratio, while using only 20 frequencies instead of 100. The details of the proposed inverse problem strategy are presented in @chap:transformations and @chap:inverse_problem. The results and performance of the model developed in this work for predicting the elastic constants of cubic materials are discussed in @chap:inverse_problem.
